...
ROS_INFO("local: %.3f, %.3f", local_x, local_y);
...
void collision_avoidance(geometry_msgs::PoseStamped& host_mocap, geometry_msgs::TwistStamped* vs,geometry_msgs::PoseStamped& host_mocap2)
{
float distance_x = (host_mocap.pose.position.x-host_mocap2.pose.position.x);
float distance_y = (host_mocap.pose.position.y-host_mocap2.pose.position.y);
float distance = sqrt(pow(distance_x,2)+pow(distance_y,2));
ROS_INFO("distance_x distance_y distance: %.2f, %.2f, %.2f", distance_x, distance_y, distance);
float err_vel;
if(distance<=1.4)
{
err_vel = 1.7-distance;
vs->twist.linear.x = err_vel*sin(atan2(distance_y,distance_x));
vs->twist.linear.y = -err_vel*cos(atan2(distance_y,distance_x));
}
//ROS_INFO("atan2: %.2f",atan2(distance_y,distance_x)/pi*180);
//ROS_INFO("vsx vsy: %.2f, %.2f", vs->twist.linear.x, vs->twist.linear.y);
}
...
int main(int argc, char **argv)
{
  ...
  dis1y = -0.5;//0.7
   ...
   for(int i = 100; ros::ok() && i > 0; --i){
    ...
    follow(vir1,host_mocap,&vs,dis1x,dis1y);
    //if(sqrt(pow((host_mocap.pose.position.x-vir1.x),2)+pow((host_mocap.pose.position.y-vir1.y),2))<=0.7)
    //{
           collision_avoidance(host_mocap,&vs,host_mocap2);
    //}
        //mocap_pos_pub.publish(host_mocap);
        }
        if(flag==1)
        {
            vir1.x = host_mocap2.pose.position.x;
            vir1.y = host_mocap2.pose.position.y;
            vir1.z = host_mocap2.pose.position.z;
            vir1.roll = -qua2eul(host_mocap2);
            dis1y = -0.7;
            ROS_INFO("roll: %f",vir1.roll/pi*180);
        int c = getch();
        //ROS_INFO("C: %d",c);
