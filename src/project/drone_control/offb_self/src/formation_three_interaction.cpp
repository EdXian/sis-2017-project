...
errx = 0.5*(vir.x - host_mocap.pose.position.x + local_x) + 0.5*(nbr_mocap.pose.position.x - host_mocap.pose.position.x + local_x1) + 0.5*(nbr2_mocap.pose.position.x - host_mocap.pose.position.x + local_x2);
erry = 0.5*(vir.y - host_mocap.pose.position.y + local_y) + 0.5*(nbr_mocap.pose.position.y - host_mocap.pose.position.y + local_y1) + 0.5*(nbr2_mocap.pose.position.y - host_mocap.pose.position.y + local_y2);
errz = 0.5*(vir.z - host_mocap.pose.position.z + dis_host.z) + 0.5*(nbr_mocap.pose.position.z - host_mocap.pose.position.z + dis_nbr.z) + 0.5*(nbr2_mocap.pose.position.z - host_mocap.pose.position.z + dis_nbr2.z);
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber host_sub = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/RigidBody1/pose", 100, host_pos);

    ros::Subscriber host_sub2 = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/RigidBody2/pose", 100, host_pos2);
	
        ros::Subscriber host_sub3 = nh.subscribe<geometry_msgs::PoseStamped>("/vrpn_client_node/RigidBody3/pose", 100, host_pos3);

    ros::Publisher local_vel_pub = nh.advertise<geometry_msgs::TwistStamped>("drone1/mavros/setpoint_velocity/cmd_vel", 10);

    ros::Publisher local_vel_pub2 = nh.advertise<geometry_msgs::TwistStamped>("drone2/mavros/setpoint_velocity/cmd_vel", 10);
	
	ros::Publisher local_vel_pub3 = nh.advertise<geometry_msgs::TwistStamped>("drone3/mavros/setpoint_velocity/cmd_vel", 10);
   ...
   for(int i = 100; ros::ok() && i > 0; --i){
	...
	//ROS_INFO("C: %d",c);
		...
		mocap_pos_pub2.publish(host_mocap2);
                mocap_pos_pub3.publish(host_mocap3);*/
		//mocap_pos_pub4.publish(host_mocap4);
    /*if(sqrt(pow((host_mocap.pose.position.x-host_mocap2.pose.position.x),2)+pow((host_mocap.pose.position.y-host_mocap2.pose.position.y),2))<=1.4)
    {
      vs.twist.linear.x = 0.5*(cos(pi/2)*(host_mocap2.pose.position.x-host_mocap.pose.position.x)+sin(pi/2)*(host_mocap2.pose.position.y-host_mocap.pose.position.y));
        vs.twist.linear.y = 0.5*(-sin(pi/2)*(host_mocap2.pose.position.x-host_mocap.pose.position.x)+cos(pi/2)*(host_mocap2.pose.position.y-host_mocap.pose.position.y));
      vs2.twist.linear.x = 0.5*(cos(pi/2)*(host_mocap.pose.position.x-host_mocap2.pose.position.x)+sin(pi/2)*(host_mocap.pose.position.y-host_mocap2.pose.position.y));
      vs2.twist.linear.y = 0.5*(-sin(pi/2)*(host_mocap.pose.position.x-host_mocap2.pose.position.x)+cos(pi/2)*(host_mocap.pose.position.y-host_mocap2.pose.position.y));

    }
    if(sqrt(pow((host_mocap.pose.position.x-host_mocap3.pose.position.x),2)+pow((host_mocap.pose.position.y-host_mocap3.pose.position.y),2))<=1.4)
    {
      vs.twist.linear.x = 0.5*(cos(pi/2)*(host_mocap3.pose.position.x-host_mocap.pose.position.x)+sin(pi/2)*(host_mocap3.pose.position.y-host_mocap.pose.position.y));
        vs.twist.linear.y = 0.5*(-sin(pi/2)*(host_mocap3.pose.position.x-host_mocap.pose.position.x)+cos(pi/2)*(host_mocap3.pose.position.y-host_mocap.pose.position.y));
      vs3.twist.linear.x = 0.5*(cos(pi/2)*(host_mocap.pose.position.x-host_mocap3.pose.position.x)+sin(pi/2)*(host_mocap.pose.position.y-host_mocap3.pose.position.y));
      vs3.twist.linear.y = 0.5*(-sin(pi/2)*(host_mocap.pose.position.x-host_mocap3.pose.position.x)+cos(pi/2)*(host_mocap.pose.position.y-host_mocap3.pose.position.y));

    }
    if(sqrt(pow((host_mocap3.pose.position.x-host_mocap2.pose.position.x),2)+pow((host_mocap3.pose.position.y-host_mocap2.pose.position.y),2))<=1.4)
    {
      vs3.twist.linear.x = 0.5*(cos(pi/2)*(host_mocap2.pose.position.x-host_mocap3.pose.position.x)+sin(pi/2)*(host_mocap2.pose.position.y-host_mocap3.pose.position.y));
        vs3.twist.linear.y = 0.5*(-sin(pi/2)*(host_mocap2.pose.position.x-host_mocap3.pose.position.x)+cos(pi/2)*(host_mocap2.pose.position.y-host_mocap3.pose.position.y));
      vs2.twist.linear.x = 0.5*(cos(pi/2)*(host_mocap3.pose.position.x-host_mocap2.pose.position.x)+sin(pi/2)*(host_mocap3.pose.position.y-host_mocap2.pose.position.y));
      vs2.twist.linear.y = 0.5*(-sin(pi/2)*(host_mocap3.pose.position.x-host_mocap2.pose.position.x)+cos(pi/2)*(host_mocap3.pose.position.y-host_mocap2.pose.position.y));
